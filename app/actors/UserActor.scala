...
class UserActor(uid: String, actorFactory: (ActorRefFactory, Props) => ActorRef, out: ActorRef) extends Actor with ActorLogging {
  ...
  val twitterFilterActor = actorFactory(context, TwitterFilterActor.props(client=self))
...
object UserActor {
  def props(uid: String, actorFactory: (ActorRefFactory, Props) => ActorRef = (actorRefFactory, props) => actorRefFactory.actorOf(props))(out: ActorRef) = Props(classOf[UserActor], uid, actorFactory, out)
}
